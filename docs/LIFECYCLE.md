...
## 0.9.60-test：退出分类与四次恢复上限

仅明确的系统 `User Request`（含退出枚举、顶层消息或详细退出消息）与轻启内主动停止取消恢复。经令牌、PID、退出时间和成功命令核实的监督自发终止用于重建，不作为用户停止。其他原因（包括升级、崩溃、资源回收、直接退出和未知原因）均保留恢复资格。

连续失败按 5/15/30/60 秒最多重试四次，最后一次仍未健康则停止并提示重新连接；稳定健康 60 秒后重置连续失败计数。累计次数保留用于诊断，不直接消耗下一轮预算。控制文件每 5 秒检查，主动停止可中断等待。启动命令失败计入同一恢复上限。

这提供有上限的异常恢复，不能保证系统永不结束监督进程，也不能绕过设备重启、撤销调试权限、工作模块不可用等环境条件。升级后若模块无法启动，会退避并记录失败；模块安装仍走正常本机连接流程。

以下为 0.9.52 的历史实现和当时实测记录，三次上限及升级停止行为已被上述策略替代。
